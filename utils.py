...
def create_optimizer(params: dict, param_groups: List, optimizer_type: str, lr: float, weight_decay: float):
    ...
    print(f"✅ Optimizer created: {optimizer_type} | lr={lr}, weight_decay={weight_decay}, momentum={momentum}")
    print(f"Parameter groups: {len(weights)} weights(decay), {len(bn_no_decay)} batchnorm(no decay), {len(biases)} biases(no decay)")
