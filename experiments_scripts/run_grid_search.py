...
from experiments_scripts.experiment_configs import (
    OPTIMIZER_PRETRAINED, OPTIMIZER_SMALL, PRETRAINED_DATASET_BASE, SMALL_CUSTOM_LOSS_BASE, 
    PRETRAINED_CUSTOM_LOSS_BASE, LAMBDA_RATES, LEARNING_RATES, MOMENTUMS, SMALL_DATASET_BASE,
    WEIGHT_DECAYS, PRETRAINED_OPTIONS
)
...
def generate_experiment_configs(base_config: Dict[str, Any], param_grid: Dict[str, List[Any]]) -> List[Dict[str, Any]]:
    """
...
def log_key_params(config):
    log("Parameters:")
    for key in ["lr", "momentum", "weight_decay", "optimizer", "augmentation", "dataset_size", "freeze_backbone_layers"]:
        if key in config:
            log(f"  {key}: {config[key]}")
    log("")

def run_experiment_from_config(config: Dict[str, Any], experiment_idx: int, total_experiments: int) -> bool:
    """
    ...
    try:
        ...
        log_error(f"Experiment '{experiment_name}' failed with error code {e.returncode}")
        return False
...
def run_grid_search(base_config: Dict[str, Any], param_grid: Dict[str, List[Any]], 
    ...
    log_experiment_options(base_config, param_grid, continue_on_error, start_time, configs)
        ...
        try:
            ...
            if success:
                ...
                if not continue_on_error:
                    log_warning(f"Stopping grid search due to failure (continue_on_error=False)")
                    break
...
def log_experiment_options(base_config, param_grid, continue_on_error, start_time, configs):
    log(f"\n{'#'*80}")
    ...
    if phase == "single":
        if base is None:
            raise click.UsageError(
                "--base is required when --phase=single"
            )
        if grid is None:
            raise click.UsageError(
                "--grid is required when --phase=single"
            )
