...
def visualize_batch(
    images, batch_dict, save_dir, predictions=None, epoch=0, is_train=True, max_imgs=4
):
    """
    ...
    for idx in range(batch_size):
        ...
        if img_mask.any():
            _draw_ground_truth_boxes(
                batch_dict, colors_dict, labels, ax, h, w, img_mask
            )
    ...
    if len(pred_boxes) > 0:
        ...
        for pred in pred_boxes:
            ...
            rect = Rectangle(
                ...
                y2 - y1,
                linewidth=2,
                edgecolor=colors_dict.get(cls, "green"),
                facecolor="none",
                linestyle="--",
    ...
    with torch.no_grad():
        for batch_idx, (X_val, y_val) in enumerate(data_loader):
            ...
            predictions = non_max_suppression(
                preds, conf_thres=CONFIDENCE_THRESHOLD, iou_thres=0.45, max_det=300
            )
            ...
            for img_idx in range(X_val.shape[0]):
                ...
                if predictions and len(predictions) > img_idx:
                    dets = predictions[
                        img_idx
                    ]  # [N, 6] tensor: x1, y1, x2, y2, conf, cls, same as before
...
def _draw_label(colors_dict, labels, img, x1, y1, conf, cls):
    ...
    cv2.rectangle(
        img, (x1, y1 - th - 4), (x1 + tw, y1), colors_dict.get(cls, GREEN), -1
    )
    cv2.putText(img, label_text, (x1, y1 - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, WHITE, 1)
