...
COLORS_1B = {
    ...
    "phase2_full_augmentation_all_images": "#16a085",
}
...
def plot_train_val_gap(experiments_data: List[Dict]):
    ...
    plt.axhline(
        ...
        linestyle="--",
        linewidth=2,
        label="Overfitting Threshold (15%)",
        alpha=0.7,
    )
...
def main():
    ...
    print("\n" + "=" * 80)
    print(
        f"PHASE {PHASE_NAME}: OPTIMIZER SELECTION - COMPREHENSIVE ANALYSIS (Mode: {args.mode.upper()})"
    )
    print("=" * 80 + "\n")
    ...
    for exp_name in PHASE_EXPERIMENTS:
        try:
            ...
            if exp_data["mode"] == "average":
                print(
                    f"  ✅ Loaded: {EXPERIMENT_LABELS[exp_name]} (averaged {exp_data['num_runs']} runs)"
                )
            else:
