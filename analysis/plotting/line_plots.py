...
def _plot_epoch(df : dict, val_total : float, label : str, color, alpha : int = 0.8, linewidth : float = 2.5):
    plt.plot(
        df["epoch"],
        val_total,
        label=label,
...
def _add_styling_epoch(ax : Axes, ylabel : str,  title : str, color:str = "black"):
    ax.set_xlabel("Epoch", fontsize=11, fontweight="bold")
    ax.set_ylabel(ylabel, fontsize=11, fontweight="bold")
    ax.set_title(
        title, fontsize=15, fontweight="bold", color=color
    )
...
def plot_loss_convergence(experiments_data: List[Dict]):
    ...
    for exp in experiments_data:
        ...
        _plot_epoch(df, val_total, exp["label"], COLORS[exp["name"]])
    ...
    _add_styling_epoch(
        ...
        title=f"Phase {PHASE_NAME}: Validation Loss Convergence Comparison",
    )
...
def plot_train_val_comparison_per_experiment(experiments_data: List[Dict]):
    ...
    for ax, exp in zip(axes, experiments_data):
        df = exp["results"]
        train_total = total_loss(df, "train")
        val_total = total_loss(df, "val")

        _plot_epoch(df, train_total, "Train Loss", "#3498db")
        _plot_epoch(df, val_total, "Val Loss", "#e74c3c")

        _add_styling_epoch(ax, "Total Loss", exp["label"], color=COLORS[exp["name"]])
...
def plot_loss_components_per_experiment(experiments_data: List[Dict]):
    ...
    for ax, exp in zip(axes, experiments_data):
        ...
        _plot_epoch(df, df["val/box_loss"], "Box Loss", "#e74c3c", linewidth=2)
        _plot_epoch(df, df["val/cls_loss"], "Class Loss", "#3498db", linewidth=2)
        _plot_epoch(df, df["val/dfl_loss"], "DFL Loss", "#2ecc71", linewidth=2)
        _plot_epoch(df, df["val/spatial_loss"], "Spatial Loss", "#9b59b6", linewidth=2)

        _add_styling_epoch(ax, "Validation Loss", exp["label"], color=COLORS[exp["name"]])
...
def plot_train_val_gap(experiments_data: List[Dict]):
    ...
    for exp in experiments_data:
        ...
        _plot_epoch(df, gap_percent, exp["label"], COLORS[exp["name"]])
